    ...
    match current_mode {
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    if let Some(commit) = app.repo.commits.get(app.ui.selected_commit_idx) {
                        app.ui.push_modal(AppMode::CommitAction(commit.hash.clone()));
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    if app.ui.settings_state.selected == 0 {
                        ...
                        app.ui.pop_modal();
                        app.ui.push_modal(AppMode::Message(format!("Amended to {}", hash)));
                    }
                }
                KeyCode::Esc | KeyCode::Char('q') => { app.ui.pop_modal(); }
                _ => {}
            }
            return false;
        }
        AppMode::Shell(input) => {
            match key.code {
                ...
                KeyCode::Esc => { app.ui.pop_modal(); }
                KeyCode::Char(c) => { 
                    if let AppMode::Shell(inp) = app.ui.current_mode_mut() { inp.push(c); }
                }
                KeyCode::Backspace => { 
                    if let AppMode::Shell(inp) = app.ui.current_mode_mut() { inp.pop(); }
                }
                _ => {}
            }
            return false;
        ...
        AppMode::InteractiveRebase => {
            if app.ui.rebase_state.editing {
                ...
                match key.code {
                    ...
                    KeyCode::Char('r') => { app.ui.rebase_state.commits[app.ui.rebase_state.selected].action = RebaseAction::Reword; }
                    KeyCode::Char('d') => { app.ui.rebase_state.commits[app.ui.rebase_state.selected].action = RebaseAction::Drop; }
                    KeyCode::Char('s') => { app.ui.rebase_state.commits[app.ui.rebase_state.selected].action = RebaseAction::Squash; }
                    KeyCode::Char('p') => { app.ui.rebase_state.commits[app.ui.rebase_state.selected].action = RebaseAction::Pick; }
                    KeyCode::Enter => {
                    ...
                    KeyCode::Char('A') => {
                        ...
                        let _ = app.ai_trigger_tx.try_send(("rename_commit".to_string(), path.to_string(), hash));
                    }
                    KeyCode::Char('E') => {
                        crate::actions::commands::execute_interactive_rebase(path, &app.ui.rebase_state.commits);
                        app.ui.pop_modal();
                        app.ui.push_modal(AppMode::Message("Rebase executed.".to_string()));
                    }
                    KeyCode::Esc | KeyCode::Char('q') => { app.ui.pop_modal(); }
                    _ => {}
                }
        ...
        AppMode::CreateBranch(input) => {
            match key.code {
                    ...
                    if !input.is_empty() {
                        match crate::git::commands::run_git(path, &["checkout", "-b", &input]) {
                            Ok(_) => {
                                ...
                                app.ui.pop_modal();
                                app.ui.push_modal(AppMode::Message(format!("Created {}", input)));
                            }
                            Err(e) => {
                                app.ui.pop_modal();
                ...
                KeyCode::Esc => { app.ui.pop_modal(); }
                KeyCode::Char(c) => { 
                    if let AppMode::CreateBranch(inp) = app.ui.current_mode_mut() { inp.push(c); }
                }
                KeyCode::Backspace => { 
                    if let AppMode::CreateBranch(inp) = app.ui.current_mode_mut() { inp.pop(); }
                }
                _ => {}
            }
            return false;
        ...
        AppMode::CodePreview(mut state) => {
            if key.code == KeyCode::Esc || key.code == KeyCode::Char('q') {
                ...
                if let AppMode::CodePreview(s) = app.ui.current_mode_mut() { *s = state; }
                return res;
            }
            return false;
        }
    ...
    match key.code {
        ...
        KeyCode::Char('i') => {
            if let Some(branch) = app.get_filtered_branches().get(app.ui.selected_branch_idx) {
                ...
                let branch_name = branch.name.clone();
                let _ = app.ai_trigger_tx.try_send(("analyze".to_string(), path.to_string(), branch_name.clone()));
        ...
        KeyCode::Char('F') if app.ui.shift_pressed && *app.ui.current_mode() == AppMode::Diff => {
             ...
             if let Some(crate::models::MergeStatus::Conflict(conflicts)) = branch.map(|b| &b.merge_status) {
                 for conflict in conflicts { let _ = app.conflict_trigger_tx.try_send((path.to_string(), conflict.clone())); }
                 app.ui.pop_modal();
                 app.ui.push_modal(AppMode::Message("AI Resolving conflicts...".to_string()));
             }
             false
        }
        _ => false
...
fn handle_search_keyboard(app: &mut App, key: KeyEvent) -> bool {
    match key.code {
        KeyCode::Enter | KeyCode::Esc => { app.ui.pop_modal(); }
...
fn handle_settings_keyboard(app: &mut App, key: KeyEvent) -> bool {
    ...
    match key.code {
        ...
        KeyCode::Char('q') | KeyCode::Esc | KeyCode::BackTab => { app.ui.pop_modal(); }
        _ => {}
    }
    false
}
        ...
        KeyCode::Enter => {
            ...
            if let Some(b) = branch {
                match app.ui.manage_selected {
                    ...
                    3 => { 
                        app.ui.pop_modal();
                        app.ui.push_modal(AppMode::Message("Rename branch coming soon!".to_string())); 
                    }
                    4 => { 
